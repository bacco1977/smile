...
def score(image_path):
    ...
    def score_from_image(image_path): 
        ...
        w = 500
        h = image.shape[0]
        middle = np.zeros((10, 2))
 
	# populate middle with the middle of face facial landmarks
        middle[0]=shape[27]
        middle[1]=shape[28]
        middle[2]=shape[29]
        middle[3]=shape[30]
        ...
        average_middle = np.average(middle,axis=0)
        left_dist= average_middle - shape[36]
        right_dist= shape[45] - average_middle
        total_dist = shape[45]-shape[36]
        left_dist_ratio = left_dist[0]/total_dist[0]
        right_dist_ratio = right_dist[0]/total_dist[0]
        
        alpha = (0.5 - left_dist_ratio*0.4)
        beta = (0.5 + right_dist_ratio*0.4) 
    
        ## This is setting where we want the eyes to be located in the output image
        eyecornerDst = [ (np.int(alpha * w ), np.int(h / 3)), (np.int(beta * w ), np.int(h / 3)) ]
